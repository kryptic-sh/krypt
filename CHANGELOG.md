...
- `krypt deps --check` asks each package's manager whether it can install the
  ...
  `dnf repoquery --whatprovides`, `brew info --json=v2` (tapping a
  `user/repo/name` tap first; a disabled formula or cask counts as missing),
  `scoop info`, `winget show`, `cargo info`) and exits 1 when any is unknown,
